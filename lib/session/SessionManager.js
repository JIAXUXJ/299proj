...
router.use(session({
    ...
    store: new MongoStore({
        ...
        collection: dbConfig.SESSION_COLLECTION
    })
}));

/*
 * Identify if a unique server-side ID has been assigned to the user making the
 * request.  If one does not exist, assign one.
 */
router.use(function(req, res, next){
	if(!req.session.ssid){
		//console.log("generating ssid for session " + req.session.id + ".");
