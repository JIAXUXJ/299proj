...
class MongoDB extends DBInterface {
    ...
    getActiveSessions(cb, exclId) {
        ...
        //may want to restrict data being returned here!
        return collection.find({_id : {$not : exclId}, gameState: 'ACTIVE'}).toArray();
    ...
    getWaitingSessions(cb) {
        ...
        //may want to restrict data being returned here!
        return collection.find({_id : {$not : exclId}, gameState: 'WAITING'}).toArray();
    ...
    getPlayerByUserName(name, cb) {
    	var collection = this._db.collection(COLLECTION_USER);
    	collection.find({username: name}).limit(1).next(function(err, doc) {
    		if(err){
    			cb(null);
    		} else {
    			cb(doc);
