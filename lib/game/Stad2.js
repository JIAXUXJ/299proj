...
newMove(gid, move, cb){
	db.getGameById(gid, function(data) {
		...
		var capture_response = _capture(data);
		
		//Update the board and scores...
		var write_data = {
			"Board": capture_response.board;
			"WhiteCaptures": gameObject.WhiteCaptures + capture_response.blackCaptured,
			"BlackCaptures": gameObject.BlackCaptures + capture_response.whiteCaptured
		};
		...
		db.appendGameMoveById(gid, move, function(push_ok){
			...
			db.updateGameById(gid, write_data, function(write_ok){
				cb(write_ok);
			});
		});
}
...
_isValid(move, gameObject){
	...
	if(_liberties(gameObject.board, move.Player, move.CoordX, move.CoordY).count 
		== 0){
		return false;
	}
