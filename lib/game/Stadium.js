...
function newGame(blackID, whiteID, size, cb){
	...
	db.newGame(g, function(data){
		if(!data){
			cb(false);
		} else if (data._id != id) {
			logger.warn('ID mismatch on creating new game: passed ' + id +', but was returned ' + data._id);
			cb(false);
		} else {
			cb(true);
		}
...
function newMove(gameID, move, cb){
	...
	// update the board
	updateBoard(move, game);
	applyMove(move.Player, move, game);
	cb(MOVE_CALLBACK_OK);
}
...
function applyMove(colour, move, game){ 
	...
	db.updateGameById(game._id,g, function(data){
		if(!data){
			cb(false);
		} else if (data._id != id) {
			logger.warn('ID mismatch on creating new game: passed ' + id +', but was returned ' + data._id);
