...
function validMove(move, game) {
	...
	for(var k = game.moves.length-1; k>=0;k--){
		...
		if(game.moves[k].Trun == move.Turn){
			if(game.moves[k].CoordX == move.CoordX && game.moves[k].CoordY == move.CoordY){
				console.log('Move in Ko: same as ' + game.moves[k].CoordX + "," + game.moves[k].CoordY);
				return false;
			}
			k = -1;
		}
...
function liberties(x, y, colour, game, checkBoard){
	...
	checkBoard[x][y] = 1;
	//check right
	if(inbounds(x+1, y, game) && checkBoard[x+1][y] == 0){
	...
	//check above
	if(inbounds(x, y+1, game) && checkBoard[x][y+1] == 0){
