...
user.post('/login', function(req, res){
	...
	auth.authenticateUser(req, function(login_good){
		if(login_good){
			res.status('200').send('Good login.').redirect('back');
		} else {
...
user.get('/logout', function(req, res){
	var pid = req.session.pid;
	db.setSessionAcct(pid, null, function(data){
		if(!data){
			logger.warning('failed to logout session ' + pid);
			res.status('503').send('Failed to logout. You have found a horrid bug!');
		} else {
			res.status('200').send('Good logout. Bye!').redirect('back');
		}
	});
});
